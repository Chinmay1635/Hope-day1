...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://hope-red-mu.vercel.app"],
...
sms_collection = client["haven_alerts"]["incoming_sms"]

# Pydantic models for SMS
...
async def health_check():
    return {"status": "running", "service": "haven-sms-webhook"}
...
@app.get("/offlineIssue")
async def get_offline_issue():
    try:
        offline_issues = list(sms_collection.find())
        serialized_issues = [serialize_object_id(issue) for issue in offline_issues]
        return JSONResponse(content=serialized_issues)
    except Exception as e:
        print("❌ Error:", e)
        raise HTTPException(status_code=500, detail=f"Error retrieving offline issues: {e}")
...
async def decode_image(image: UploadFile = File(...)):
    try:
        ...
        decoded_text = decode_text_from_image(img)

        if decoded_text:
            return {"decoded_text": decoded_text}
        return JSONResponse(status_code=400, content={"message": "No hidden text found!"})
    except Exception as e:
